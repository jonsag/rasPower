...
def build_elpris_url(year, month, day):
    ...
    path = (
        config.get("elpris", "elpris_api").strip()
        + "/"
        + year
        + "/"
        + month
...
# temperatur.nu #
#################
temperatur_SCHEME = os.environ.get(
    "API_SCHEME", config.get("temperatur", "temperatur_scheme").strip()
)
...
def build_temperatur_url(s_year, s_month, s_day, e_year, e_month, e_day):
    ...
    path = (
        config.get("temperatur", "temperatur_api").strip()
        + "?p="
        + config.get("temperatur", "temperature_location").strip()
        + "&cli="
        + config.get("temperatur", "temperature_client").strip()
        + "&data"
        + "&start="
        + s_year
        + "-"
...
# open-meteo.com #
##################
openmeteo_SCHEME = os.environ.get(
    "API_SCHEME", config.get("openmeteo", "openmeteo_scheme").strip()
)
...
openmeteo_latitude = float(config.get("openmeteo", "openmeteo_latitude").strip())
openmeteo_longitude = float(config.get("openmeteo", "openmeteo_longitude").strip())

openmeteo_hourly = config.get("openmeteo", "openmeteo_hourly").replace(" ", "")
openmeteo_daily = config.get("openmeteo", "openmeteo_daily").replace(" ", "")

openmeteo_ws_unit = config.get("openmeteo", "openmeteo_ws_unit").strip()

openmeteo_timezone = config.get("openmeteo", "openmeteo_timezone").strip()
openmeteo_forecast_days = int(
    config.get("openmeteo", "openmeteo_forecast_days").strip()
)

openmeteo_tilt = int(config.get("openmeteo", "openmeteo_tilt").strip())
openmeteo_azimuth = int(config.get("openmeteo", "openmeteo_azimuth").strip())


def build_openmeteo_url(
    latitude, longitude, hourly, daily, ws_unit, timezone, days, tilt, azimuth
):

    path = (
        config.get("openmeteo", "openmeteo_api").strip()
        + "?latitude="
        + str(latitude)
        + "&longitude="
        + str(longitude)
