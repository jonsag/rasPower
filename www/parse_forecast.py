...
def parse_openmeteo_forecast(no_days):
    ...
    if response_code == 200:
        ...
        for x in range(len(parse_json[0]["hourly"]["time"])):
            ...
            sql = f"INSERT INTO {db_name}.forecast(time, temp, rain, wind, gti_instant) VALUES ('{time[:10]} {time[11:16]}', {temp}, {rain}, {wind}, {gti_instant}) ON DUPLICATE KEY UPDATE temp = {temp}, rain = {rain}, wind = {wind}, gti_instant = {gti_instant}"
            # print(sql)
        ...
        for y in range(len(parse_json[0]["daily"]["time"])):
            ...
            sql = f"INSERT INTO {db_name}.sun(day, sunrise, sunset) VALUES ('{day}', '{sunrise[:10]} {sunrise[11:16]}', '{sunset[:10]} {sunset[11:16]}') ON DUPLICATE KEY UPDATE sunrise = '{sunrise[:10]} {sunrise[11:16]}', sunset = '{sunset[:10]} {sunset[11:16]}'"
            # print(sql)
...
def arguments(argv):
    ...
    for opt, arg in opts:
        if opt == "-h":
            print(f"{os.path.basename(sys.argv[0])} -n <number of days>")
            sys.exit()
