...
def main():
    ...
    for welcome_a, welcome_b in pairs(infinite_shuffle(welcome)):
        ...
        widget_a = make_snapshot(
            device.width, device.height, welcome_a, fonts, color_a)
        widget_b = make_snapshot(
            device.width, device.height, welcome_b, fonts, color_b)

        while True:
            posn_a = random_point(
                virtual.width - device.width, virtual.height - device.height)
            posn_b = random_point(
                virtual.width - device.width, virtual.height - device.height)
